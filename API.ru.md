...
два, плюс обработчик: `proxy` на апстрим, `static` с файлом пространства (`static_file` — имя
набора вида content) или возврат ответа.
...
| `GET`, `PUT /api/<scope>/haproxy` | настройки балансировщика; в ответе ещё `entries` — входы, выведенные из портов пространства |
...
Балансировщик слушает каждый порт пространства, который узлы отдают в сеть, кроме порта панели и
портов на loopback; смена портов делает канал `haproxy` отставшим так же, как смена настроек. Порт
с TLS и порт с PROXY protocol проходят насквозь в режиме tcp, с обычным портом балансировщик
говорит по HTTP. Узлы проверяются соединением, если `backend.check.path` не называет путь здоровья для
обычных портов. Настройки задают только то, где haproxy слушает: `entry.addresses` — адреса
машины, `entry.ports` — порт входа перед портом узла, когда они разные (`{"8080": 80}`); оба пишет
установщик. Порт пространства с тем же номером, что такой порт входа, входа не получает и помечен в
`entries` полем `taken`. `backend.servers` — узлы по имени и адресу, порт берётся у входа.
