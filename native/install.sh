#!/bin/sh
# Установка Placitum без Docker: весь контур на одной машине с Debian 13.
#
#     sudo sh install.sh                 установить; спросит то, чего ещё нет в ответах
#     sudo sh install.sh --reconfigure   переспросить всё: текущие ответы станут умолчаниями
#     sudo sh install.sh --defaults      без вопросов: чего нет в ответах, берётся умолчание
#     sudo sh install.sh status          что запущено
#     sudo sh install.sh check           чистая установка глазами пользователя
#
# Ответы -- /etc/placitum/placitum.conf. Повторный запуск и сборка образа ставят
# по ним ту же установку без вопросов. Пароль admin панели в ответах не хранится:
# без терминала он берётся из PLC_PANEL_PASSWORD, пусто -- генерируется и
# показывается один раз.
#
# Рядом со скриптом -- packages/ из native/build.sh. Инфраструктура ставится из
# репозиториев: Debian (PostgreSQL, Redis), nginx.org (nginx той версии, против
# которой собран модуль), ClickHouse и NodeSource.
...
# Инспекторы в порядке вопросов. У auth и captcha есть ещё формы (*-http) -- по одной.
INSPECTORS="ip modsec json counter action rewrite cookie auth captcha"

# Порты внутри машины, занятые контуром: трафику их не отдать.
...
# quietly <что> <команда…>: вывод -- в журнал, на экран -- строка шага. Функции,
# которые сюда передаются, проверяют каждый свой шаг сами: внутри условия if
# оболочка не прерывает их по set -e.
quietly() {
    what=$1
    shift
    started=$(date +%s)
    ...
    printf 'сбой\n'
    log_tail
    die "$what: не удалось, журнал целиком -- $log"
}

# wait_for <секунд> <команда…>: ждать, пока команда не пройдёт.
wait_for() {
    left=$1
    shift
...
why() {
    case "$1" in
        is_name)      echo "латиница в нижнем регистре, цифры и дефис" ;;
        is_count)     echo "число от 1 до 32" ;;
        is_workers)   echo "auto или число" ;;
        is_mb)        echo "мегабайты, не меньше 32" ;;
        is_http_port) echo "порт от 1 до 65535, кроме занятых контуром: $INTERNAL_PORTS" ;;
        is_bind)      echo "0.0.0.0, 127.0.0.1 или адрес IPv4 этой машины" ;;
    esac
}

# ask <переменная> <проверка> <умолчание> <вопрос>
ask() {
...
# copies <инспектор>: сколько копий ответили.
copies() {
...
answers() {
    ...
    ask PLC_NODE_ID       is_name      edge-01   "Имя узла: в панели и в аудите"
    ask PLC_HTTP_PORT     is_http_port 80        "Порт трафика HTTP"
    ask PLC_PANEL_BIND    is_bind      127.0.0.1 "Адрес панели: 127.0.0.1 -- только с этой машины, 0.0.0.0 -- со всех, или адрес интерфейса"
    ask PLC_NGINX_WORKERS is_workers   auto      "Воркеров nginx: auto -- по числу ядер"

    for name in $INSPECTORS; do
        ask "PLC_COPIES_$(upper "$name")" is_count 1 "Копий инспектора $name"
    done

    ask PLC_REDIS_EXCHANGE_MB is_mb 512 "Память обменника Redis (объекты запросов), МБ"
    ask PLC_REDIS_INTERNAL_MB is_mb 256 "Память внутреннего Redis (поколения, корзины), МБ"
...
ask_panel_password() {
    if [ -n "${PLC_PANEL_PASSWORD:-}" ]; then
        ...
        note "пароль admin панели: из PLC_PANEL_PASSWORD"
        return 0
    fi
    ...
    while :; do
        first=$(read_secret "Пароль admin панели (Enter -- прежний или сгенерировать): ")
        [ -n "$first" ] || break
...
preflight() {
    ...
    [ "$(id -u)" -eq 0 ] || die "нужен root: sudo sh $0"

    # shellcheck disable=SC1091
    . /etc/os-release
    [ "${VERSION_CODENAME:-}" = trixie ] ||
        die "установка рассчитана на Debian 13 (trixie), здесь ${PRETTY_NAME:-неизвестная система}"
    [ "$(dpkg --print-architecture)" = amd64 ] || die "нужна amd64"

    set -- "$here"/packages/placitum-common_*.deb
    [ -f "$1" ] || die "нет пакетов в $here/packages: их собирает native/build.sh"

    note "$PRETTY_NAME, ядер $(nproc), памяти $(awk '/MemTotal/ {print int($2 / 1024)}' /proc/meminfo) МБ"
    note "пакетов Placitum: $(ls "$here"/packages/*.deb | wc -l), $here/packages"
}
...
repos() {
    ...
    quietly "ключи nginx.org, ClickHouse, NodeSource" fetch_keys

    cat > /etc/apt/sources.list.d/placitum.list <<EOF
# Placitum (install.sh): nginx той версии, против которой собран модуль; ClickHouse; Node.js $NODE_MAJOR.
...
packages() {
    ...
    quietly "пакеты Placitum" apt-get install -y --no-install-recommends --allow-downgrades \
        "$here"/packages/*.deb
}
...
secrets_step() {
    ...
    if [ -f "$secrets/passwords" ]; then
        note "пароли инфраструктуры на месте: $secrets/passwords"
    else
        (
            umask 077
            {
        ...
        note "пароли инфраструктуры выпущены: $secrets/passwords"
    fi

    # shellcheck disable=SC1091
    . "$secrets/passwords"

    quietly "ключ контура и ключи подписи" env MINIO_ROOT_USER=waf MINIO_ROOT_PASSWORD="$MINIO_ROOT_PASSWORD" \
        sh /usr/share/placitum/bootstrap/secrets.sh "$secrets"

    # secrets.sh ставит режимы под Docker (каталог 0700, файлы 0644). Здесь файлы
    # читает группа placitum, остальным -- ничего; пароли -- только root.
...
# subst <шаблон> <куда> <sed-выражения…>: шаблон из core с адресами этой машины.
# Адрес Docker, оставшийся в файле, -- ошибка установки, а не предупреждение.
subst() {
    src=$1 dst=$2
    shift 2
    sed "$@" "$src" > "$dst"
    ...
    if grep -Eq 'redis://redis|nats://nats|minio:9000|/run/secrets|0\.0\.0\.0:(4222|8222)|store_dir: /data' "$dst"; then
        die "$dst: остались адреса Docker"
    fi
}

redis_conf() {
    cat > "$etc/redis/$1.conf" <<EOF
# Placitum (install.sh): Redis $1. noeviction -- тихое вытеснение ключа хуже
# громкого отказа (docs/body-storage.md). Состояние восстановимо: на диск ничего.
bind 127.0.0.1 -::1
port $2
protected-mode yes
maxmemory ${3}mb
...
<!-- Placitum (install.sh): пользователь логгера и поиска. -->
<clickhouse>
    <users>
        <waf>
...
EOF
    ...
    printf '# Имя узла для модуля: ведёт PLC_NODE_ID, файл переписывает install.sh.\nwaf_node_id %s;\n' \
...
database() {
    ...
    quietly "PostgreSQL отвечает" wait_for 60 runuser -u postgres -- pg_isready -q

    if [ "$(psql_q -tAc "SELECT 1 FROM pg_roles WHERE rolname = 'waf'")" = 1 ]; then
        psql_q -c "ALTER ROLE waf LOGIN PASSWORD '$POSTGRES_PASSWORD'" >> "$log" 2>&1 || die "роль waf: пароль"
        note "роль waf на месте, пароль сверен"
    else
        psql_q -c "CREATE ROLE waf LOGIN PASSWORD '$POSTGRES_PASSWORD'" >> "$log" 2>&1 || die "роль waf"
        note "роль waf заведена"
    fi

    if [ "$(psql_q -tAc "SELECT 1 FROM pg_database WHERE datname = 'waf'")" = 1 ]; then
        note "база waf на месте"
    else
        psql_q -c "CREATE DATABASE waf OWNER waf" >> "$log" 2>&1 || die "база waf"
        note "база waf заведена"
    fi

    # Суперпользователем waf здесь не становится, как в образе postgres. Расширения
    # схемы (pgcrypto) доверенные: их ставит сам владелец базы, и расширение
    # принадлежит ему. Поставленное заранее от postgres ломает baseline на
    # COMMENT ON EXTENSION: «must be owner of extension».
}
...
infra() {
    ...
    # Свой Redis пакета занял бы 6379: у контура два экземпляра со своими конфигами.
    systemctl disable --now redis-server >> "$log" 2>&1 || true
    systemctl daemon-reload

    quietly "Redis: обменник 6379 и внутренний 6380" \
        systemctl enable --now placitum-redis@exchange placitum-redis@internal
    quietly "NATS" systemctl enable --now placitum-nats
    quietly "потоки JetStream" env NATS_URL=nats://127.0.0.1:4222 PATH="/usr/lib/placitum/bin:$PATH" \
    ...
    quietly "ClickHouse отвечает" wait_for 90 \
        clickhouse-client --user waf --password "$CLICKHOUSE_PASSWORD" --query "SELECT 1"

    quietly "MinIO" systemctl enable --now placitum-minio
    quietly "бакеты архива и правила удаления" buckets
}

# --- контроллер ------------------------------------------------------------

# as_placitum <команда…>: от пользователя placitum в каталоге контроллера, с его окружением.
as_placitum() {
    (
...
# Подменяет его root: процесс контроллера свои файлы не пишет, так что пин
# по-прежнему не приходит из API.
pin() {
    ...
    if grep -rqF "$new" "$dist"; then
        note "пин отпечатка в панели: $new"
        return 0
    fi

    files=$(grep -rlF "$old" "$dist") || die "в сборке панели нет пина $old"

    for file in $files; do
        sed -i "s|$old|$new|g" "$file"
    done

    grep -rqF "$new" "$dist" || die "пин отпечатка не вписался в панель"
    note "пин отпечатка в панели: $new (в сборке был $old)"
}
...
nginx_stub() {
    ...
    if grep -q ngx_http_waf_module /etc/nginx/nginx.conf 2>/dev/null; then
        note "nginx: на поколении контроллера"
        return 0
    fi

    cat > /etc/nginx/nginx.conf <<'EOF'
# Заглушка узла до первого поколения контроллера, как entrypoint образа узла:
# поколение перезапишет файл. Свой порт, чтобы не спорить с тем, что приедет.
events {}
...
EOF
    note "nginx: заглушка до первого поколения"
}
...
services() {
    ...
    for name in $INSPECTORS; do
        ...
        # Копии сверх ответа остались от прошлой установки: остановить и снять.
        for unit in $(systemctl list-units --all --plain --no-legend "placitum-$name@*.service" | cut -d' ' -f1); do
            ...
            if [ "$k" -gt "$n" ] 2>/dev/null; then
                ...
                note "снята лишняя копия: $unit"
            fi
        done
    done

    # shellcheck disable=SC2086
    quietly "юнитов: $(printf '%s\n' $list | wc -l)" systemctl enable --now $list
    quietly "контроллер отвечает" wait_for 120 curl -fsS http://127.0.0.1:8080/healthz
}
...
panel() {
    ...
    quietly "порт трафика и воркеры nginx" as_placitum env PLC_HTTP_PORT="$PLC_HTTP_PORT" \
    ...
    if ! result=$(printf '%s' "$panel_pass" | as_placitum env PANEL_ADMIN=ensure \
        ...
        printf 'сбой\n'
        log_tail
        die "панель за калиткой не встала, журнал -- $log"
    fi

    printf 'готово, %s\n' "$(elapsed "$started")"

    case "$result" in
        created)       note "admin заведён с введённым паролем" ;;
        updated)       note "пароль admin сменён" ;;
        kept)          note "admin уже есть, пароль прежний" ;;
        "generated "*) generated=${result#generated } ;;
        *)             die "шаг панели ответил неожиданно: $result" ;;
    esac
}
...
publish() {
    ...
    if ! result=$(as_placitum node bootstrap/publish.mjs 2>> "$log"); then
        printf 'сбой\n'
        log_tail
        die "каналы конфигурации не сошлись, журнал -- $log"
    fi
...
summary() {
    ...
    if [ -n "$failed" ]; then
        ...
        note "процессов placitum: $active, упавших нет"
    fi

    if [ "$PLC_PANEL_BIND" = 0.0.0.0 ]; then
        for addr in $(ip -o -4 addr show | awk '$2 != "lo" {print $4}' | cut -d/ -f1); do
            note "панель:  http://$addr:8081"
        done
    else
        note "панель:  http://$PLC_PANEL_BIND:8081"
    fi

    if [ -n "$generated" ]; then
        note "вход:    admin / $generated -- пароль нигде не сохранён, запишите его"
    else
        note "вход:    admin"
    fi

    note "API:     http://127.0.0.1:8080 -- без калитки, только с этой машины (снаружи -- ssh -L)"
    note "трафик:  порт $PLC_HTTP_PORT"
    note "ответы:  $conf"
    note "журнал:  $log"
}
