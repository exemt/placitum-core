...
- панель раздаёт узел за проверкой входа входа (`auth`), и браузер попадает в API через ту же проверку входа.
...
2. **Применить** — `POST .../send` по этому каналу. Контроллер собирает то, что есть, издаёт новое
...
| Канал | Что передаёт | Применение | Изданное поколение |
| --- | --- | --- | --- |
...
В ответ на применение приходит изданное поколение:
...
хеш черновика, хеш и версия изданного, и каждый получатель с тем, что он докладывает (`ok`, `stale`,
...
Применить всё после пачки правок:
