...
- панель раздаёт узел за калиткой входа (`auth`), и браузер попадает в API через ту же калитку.
...
| `kv_unavailable` | шина недоступна, рассылать некуда |

## Сохранить и разослать

Запись меняет базу контроллера, и только её. Узел и инспекторы продолжают работать по тому, что у
них уже есть. Рассылка — отдельный явный шаг:

1. **Сохранить** — `POST`, `PUT` или `DELETE` по объектам канала.
2. **Разослать** — `POST .../send` по этому каналу. Контроллер собирает то, что есть, издаёт новое
...
4. **Проверить** — `GET /api/<scope>/convergence` сводит вместе сохранённое, разосланное и то, что
   докладывает каждый участник.

Каналы независимы: правила для инспектора `modsec` едут своим ходом, конфигурация узла — своим.

| Канал | Что везёт | Рассылка | Изданное поколение |
| --- | --- | --- | --- |
...
В ответ на рассылку приходит изданное поколение:
...
рассылка ничего не делает.

У сводки применения одна лампа на всё пространство (`green`, `yellow`, `red`) и по записи на канал:
хеш черновика, хеш и версия изданного, и каждый участник с тем, что он докладывает (`ok`, `stale`,
...
| `GET /api/fleet` | снимок всего живого: агенты узлов и их воркеры, инспекторы, обменники, службы, трафик по маршрутам |
| `POST /api/fleet/forget` | забыть процессы по именам, когда их контейнеров уже нет |
| `GET /api/<scope>/convergence` | что сохранено, что разослано, что докладывает каждый участник |
| `POST /api/<scope>/convergence/refresh` | пересчитать; `?channel=nginx` — один канал |

Участник парка бывает `up` или `degraded` и исчезает, когда перестаёт докладывать. В каждой записи
...
| `GET /api/<scope>/catalog` | всё одним куском: ответы отказа, форматы журнала, наборы, инспекторы, subject'ы, профили, страницы, хранилища тел, апстримы |
| `GET /api/<scope>/content-types` | типы содержимого для наборов вида `content` |
| `GET /api/<scope>/crypto` | открытый ключ контура, его алгоритм и отпечаток |
...
Уровни журналирования применяются сразу по всей установке, без рассылки: документ уходит на шину, и
...
`raw: true` у пространства, сервера или маршрута значит, что его текст пишут руками в `raw_nginx`,
...
## Сертификаты и обменник

Сертификат никогда не передаётся полем. Сначала файл кладут в обменник, зашифрованным ключом
контура, и уже сертификат ссылается на положенные объекты.
...
| `GET`, `POST /api/<scope>/certificates` | сертификаты пространства; создать из объектов обменника |
...
Инспектор — процесс, который судит запросы. В пространстве лежит их каталог: имя, которым инспектора
зовут в конфигурации, subject шины, на котором процесс слушает, и фазы, в которых он работает.
...
| `GET /api/<scope>/inspectors/declared` | каких инспекторов конфигурация зовёт на самом деле, с каким профилем и знает ли их каталог |
...
| `POST /api/<scope>/<имя>/send` | разослать профили этого инспектора |
...
(`409 fast_path_gated`), пока маршрут зовёт инспектора по условию: условие может проскочить мимо
проверки групп, и калитка не удержит.
...
| `POST /api/<scope>/rules/compile` | собрать, не рассылая; в ответ счётчики и хеш |
| `POST /api/<scope>/rules/send` | собрать и разослать |
...
правил до того, как он доедет до узла.
...
| `POST /api/<scope>/ip-profiles/send` | разослать профили |
...
| `GET /api/geo/files/<kind>` | загруженный файл: хеш, размер, дата сборки, издан ли он |
...
## Ответы отказа, хранилища тел, форматы журнала

| Ручка | Что делает |
| --- | --- |
| `GET`, `POST /api/<scope>/deny-responses` | чем узел отвечает на отказ |
...
Ответ отказа приходит с полем `uses`: все места конфигурации, где он назван. Хранилище тел
...
| `POST /api/<scope>/config/send` | собрать, проверить и разослать |
...
несохранённое на сохранённое и возвращает только этот блок, с `errors` и `warnings` вместо отказа.
Ничего не сохраняется и никуда не уезжает.
...
| `POST /api/<scope>/agent/send` | разослать их |
...
| `POST /api/<scope>/haproxy/send` | разослать их |
| `GET /api/<scope>/haproxy/desired` | изданное поколение |

Рассылка, которой нечего менять, оставляет прежнюю версию и просто подтверждает поколение.
...
| Ручка | Что отдаёт |
| --- | --- |
| `GET /api/search/audit` | записи аудита: по одной на проверенный запрос |
...
curl -s -X POST $API/$SCOPE/config/send     # собрать, проверить, разослать
```
...
Канал остаётся `stale`, пока участник работает по прежнему поколению, и становится `failed`, если
он попробовал и не смог. Кто именно — скажет `GET /api/fleet`.

Разослать всё после пачки правок:
...
Рассылка, которой нечего рассылать, безвредна: поколение и его версия останутся прежними.
