...
the version of the installation. `#develop` is current development; production installations use
...
- **Windows host (Docker Desktop)**: the bake builder does not understand git source URLs on Windows
  (`failed to evaluate path "https://…git#develop"`). The installation targets Linux; on Windows set
